...
import { Play, FileText, BarChart2, Sparkles, Wand2 } from 'lucide-react';
...
export default function StudyPlan() {
  ...
  const fetchPlan = async () => {
    try {
      ...
      // The API returns an array, we sort it or just use it.
  ...
  // Helper to format 8.5 to "08:30 AM"
  ...
  let currentStartHour = 8; // Start at 08:00 AM
          ...
          <div className="space-y-8 flex flex-col relative z-10">
            {loading ? (
              ...
              plan.map((task, idx) => {
                ...
                currentStartHour += task.allocatedHours; // update for next item
        ...
        <div className="lg:col-span-4">
          <div className="bg-surface border border-surface-border rounded-[32px] p-8 sticky top-6">
            <div className="flex items-center gap-3 mb-10">
              ...
              <h2 className="text-xl font-bold text-white tracking-tight">Plan Analytics</h2>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-10">
              <div className="bg-surface-sidebar rounded-2xl p-5 border border-surface-border">
                ...
                <span className="text-2xl font-bold text-white leading-none">{plan.length}</span>
              </div>
              <div className="bg-surface-sidebar rounded-2xl p-5 border border-surface-border">
                <span className="text-[9px] font-bold text-text-muted uppercase tracking-widest block mb-2">Intensity</span>
                <span className="text-2xl font-bold text-white leading-none">High</span>
              </div>
            </div>

            <div className="relative rounded-2xl overflow-hidden border border-surface-border group">
              <div className="absolute inset-0 bg-gradient-to-t from-[#0a0b10] to-transparent z-10"></div>
              {/* Fake chart visualization because ML analytics would require dedicated endpoints */}
              <div className="h-28 w-full bg-[#111218] flex items-end opacity-50 px-2 pt-4 gap-[2px]">
              ...
              <div className="absolute bottom-4 left-4 z-20">
                ...
                <span className="text-sm font-semibold text-white">Neural plasticity peaking</span>
              </div>
            </div>
          </div>
        </div>
